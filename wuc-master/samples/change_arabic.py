...
class ChangeArabic(object):
    def __init__(self):
        self.except_word = [
            ...
            u'페이백', u'내일', u'어린이', u'청구', u'사회기능', u'십시오', u'이분', u'하나만', u'타일', u'약정일'
        ]
        self.unit_dict = {
            1: ['원', '개', '년', '월', '일', '분', '동', '호', '종', '병', '회', '길', '대', '층', '%', '시', '분'],
            2: ['개월', '프로', '번지', '단지', '번길', '키로', '주일', '번째'],
            3: ['퍼센트', '퍼센터', '퍼센투', '퍼센토']
        }
        self.replace_dict = {
            '이십니까': ' 이 십 니까',
        ...
        self.number_unit_re = re.compile(
            (r'(일|이|삼|사|오|육|칠|팔|구|영|십|백|천|만|억|하나|둘){2,}'.decode('utf-8') + '({0})'.format(unit_str).decode('utf-8')))
        self.number_re = re.compile(
            r'(일|이|삼|사|오|육|칠|팔|구|영|십|백|천|만|억|하나|둘){3,}'.decode('utf-8'))
        self.number_check_re = re.compile(
            r'(일|이|삼|사|오|육|칠|팔|구|영|십|백|천|만|억|하나|둘){2,}'.decode('utf-8'))
    ...
    def change_arabic(target_txt):
        ...
        for before, after in replace_dict.items():
            target_txt = target_txt.replace(before.decode('utf-8'), after.decode('utf-8'))
        arabic_txt = str()
        count_type = str()
        current_size = 1
        current_unit_size = 1
        for idx in range(len(target_txt)-1, -1, -1):
            value = target_txt[idx].encode('utf-8')
            if value in number_dict.keys():
                if count_type == 'size_size':
                    ...
                    arabic_txt = target_txt[:idx+1].encode('utf-8') + str(arabic_txt)
                    break
                else:
                    count_type = 'number'
                ...
                if count_type in ['size_size', 'size_number'] and value in ['십', '백', '천']:
                    ...
                    break
        return str(arabic_txt).decode('utf-8')
        ...
        for value in iter_result:
            ...
            for unit_len in range(3, 0, -1):
                ...
                for unit in unit_list:
                    if unit.decode('utf-8') in unicode_text[idx_start:idx_end]:
                        if unit == '일':
...
if __name__ == '__main__':
    ...
    if arguments.target_str:
        ...
        for dir_path, sub_dirs, files in w_ob:
            for file_name in files:
                ...
                for line in target_file:
                    try:
                        print >> output_file, change.change_arabic_text(
                            line.decode(arguments.encode_type).encode('utf-8')).strip()
                    except Exception:
                        pass
                target_file.close()
                output_file.close()
        ...
        for line in target_file:
            print(change.change_arabic_text(line.decode(arguments.encode_type).encode('utf-8')))
        target_file.close()
